...
export function setupAtmosphere(scene: THREE.Scene, profile?: ColorProfile) {
  ...
  const ambientIntensity = Math.max(0.08, profile?.ambientIntensity ?? 0.05);
  ...
  const baseline = new THREE.AmbientLight(0x1f1f29, 0.12);
  ...
  const hemi = new THREE.HemisphereLight(hemiSky, hemiGround, 0.18);
  scene.add(hemi);
  ...
  const baseDensity = profile?.fogDensity ?? 0.042;
  const density = isMobile ? baseDensity * 1.08 : baseDensity;
